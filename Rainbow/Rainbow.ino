...
uint8_t thisdelay = 7;                                        // A delay value for the sequence(s)
uint8_t thishue = 0;                                          // Starting hue value.
uint8_t deltahue = 250;                                        // Hue change between pixels.
...
//  LEDS.addLeds<LED_TYPE, LED_DT, LED_CK, COLOR_ORDER>(leds, NUM_LEDS);   // For APA102 or WS2801
  ...
  set_max_power_in_volts_and_milliamps(5, 2);               // FastLED 2.1 Power management set at 5V, 500mA
} // setup()
