...
uint8_t thisdelay = 22;                                        // A delay value for the sequence(s)
...
void rainbow_march() {                                        // The fill_rainbow call doesn't support brightness levels
  heatindex++;                                                  // Increment the starting hue.
  //fill_rainbow(leds, NUM_LEDS, thishue, deltahue);            // Use FastLED's fill_rainbow routine.
  //fill_gradient_RGB(leds, NUM_LEDS, CRGB(0,0,200), CRGB(150,0,20));  // up to 4 CRGB (or long) values
  for(int i = 0; i < NUM_LEDS; i++) {
    leds[i] = ColorFromPalette( myPal, heatindex+((i*255)/NUM_LEDS)); // normal palette access
  }
  //delay(20);
} // rainbow_march()
